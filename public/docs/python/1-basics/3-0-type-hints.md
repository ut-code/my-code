---
id: python-basics-type-hints
title: 型ヒント（Type Hints）の紹介
level: 2
question:
  - 型ヒントを付けると、どのようなバグを事前に発見できますか？
  - 型ヒントと異なる型の値を代入してもエラーにならないなら、型ヒントは何のためにあるのですか？
  - '`user_id = "abc-789"` のように型ヒントと異なる型を代入した場合、IDEや静的解析ツールは警告を出してくれますか？'
  - すべての変数に型ヒントを付けるべきですか？
  - 型ヒントを付けないと、コードにどのような影響がありますか？
---

## 型ヒント（Type Hints）の紹介

動的型付けは柔軟ですが、コードが大規模になると変数がどの型を期待しているのかが分かりにくくなることがあります。そこで導入されたのが**型ヒント**です。これは、変数や関数の引数、返り値に「期待される型」を注釈として付与する機能です。

`変数名: 型` のように記述します。

**重要な注意点:** 型ヒントはあくまで「ヒント」であり、**Pythonの実行エンジンはこれを強制しません**。つまり、型ヒントと異なる型の値を代入してもエラーにはなりません。

```python-repl
>>> # 型ヒントを付けた変数宣言
>>> user_name: str = "Alice"
>>> user_id: int = 123
>>> type(user_id)
<class 'int'>
>>> # int型とヒントを付けたが、文字列を代入できてしまう
>>> user_id = "abc-789"
>>> type(user_id)
<class 'str'>
```

型ヒントは、コードの可読性を高めたり、MyPyのような静的解析ツールや統合開発環境（IDE）がコードのバグを事前に発見したりするために利用されます。
