---
id: python-generators-decorators-generator-yield
title: ジェネレータ関数とyieldキーワード
level: 2
question:
  - ジェネレータ関数は普通の関数と何が違いますか
  - '`return` と `yield` はどのように使い分ければ良いですか'
  - '`yield` が関数の実行を一時停止するとありますが、具体的にどこで再開するのですか'
  - ジェネレータを使うとメモリ効率が良いとは、具体的にどのようなメリットがあるのですか
  - '`fib_generator(5)` を呼び出したとき、すぐにフィボナッチ数列の計算が始まるわけではないのですか'
---
