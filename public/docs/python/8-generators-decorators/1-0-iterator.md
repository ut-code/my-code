---
id: python-generators-decorators-iterator
title: イテレータとイテラブル
level: 2
question:
  - イテラブルとイテレータの主な違いは何ですか
  - なぜ `for` ループの裏側でイテレーションプロトコルが動いていることを知る必要があるのですか
  - '`__iter__()` や `__next__()` メソッドは自分で定義する必要があるのですか'
  - '`iter()` 関数や `next()` 関数は普段のプログラミングでどのような時に使うのですか'
  - '`StopIteration` 例外が発生したら、自分で何か処理を書く必要がありますか'
---
