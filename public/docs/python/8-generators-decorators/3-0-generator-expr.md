---
id: python-generators-decorators-generator-expr
title: ジェネレータ式
level: 2
question:
  - ジェネレータ式はリスト内包表記と比べていつ使うのが適切ですか
  - '`list(gen_exp)` のようにジェネレータ式の結果をリストに変換すると、メモリ効率のメリットは失われますか'
  - ジェネレータ式は一度値を使い切ると、もう一度同じジェネレータ式から値を取り出すことはできないのですか
---
