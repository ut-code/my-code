---
id: python-file-io-open
title: open()関数によるファイルのオープン
level: 2
question:
  - ファイルパスはどのように指定すれば良いですか？絶対パスと相対パスの違いは何ですか？
  - '`''r''`, `''w''`, `''a''`, `''x''` などモードがいくつかありますが、それぞれどのような場面で使うのが適切ですか？'
  - '`encoding=''utf-8''` はなぜ重要なのでしょうか？他のエンコーディングを使うことはありますか？'
  - '`f.close()` を呼び出さないと、具体的にどのような問題が発生しますか？'
---
