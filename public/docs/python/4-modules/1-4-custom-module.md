---
id: python-modules-custom-module
title: 自作モジュールの作成と利用
level: 3
question:
  - 自作モジュールはどのような場合に作成しますか？
  - '`.py`ファイルを作るだけでモジュールになるのですか？'
  - '`utils.py`と`main.py`は同じディレクトリに置く必要がありますか？'
  - '`if __name__ == "__main__":`は何のために書くのですか？'
  - '`__name__`という特殊変数とは何ですか？'
  - モジュールとしてインポートされた際には実行したくないテストコードとは、どのようなコードのことですか？
---
