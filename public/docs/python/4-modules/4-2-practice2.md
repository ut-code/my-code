...
level: 3
question:
  - '`YYYY-MM-DD`形式とは具体的にどのような形式ですか？'
  - '`strftime(''%Y-%m-%d'')`の`%Y`は何を表していますか？'
  - '`json.dumps()`の`indent`引数に`2`を指定すると、出力はどのように変わりますか？'
  - '出力例にある`"status": "完了"`という項目は、手順のどこで追加すれば良いですか？'
  - 辞書に含める情報の項目は、例に示されているもの以外にも追加しても良いですか？
---
