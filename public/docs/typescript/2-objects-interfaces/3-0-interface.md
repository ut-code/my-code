---
id: typescript-objects-interfaces-interface
title: 'インターフェース (interface): オブジェクトの「形状」を定義する'
level: 2
question:
  - インターフェースはクラスとどのように関連しますか。
  - インターフェースの定義で`=`が不要なのはなぜですか。
  - インターフェースで定義したオブジェクトに、定義されていないプロパティを追加しようとするとどうなりますか。
  - '`interface`キーワードは何かの略ですか。'
---
