---
id: typescript-objects-interfaces-object-types
title: 'オブジェクトの型付け: インラインでの定義'
level: 2
question:
  - 変数宣言時に直接オブジェクトの型を定義するメリットは何ですか。
  - 同じ構造のオブジェクトを複数作る場合にコードが冗長になるのは具体的にどういうことですか。
  - '`const`ではなく`let`で宣言した場合でも、型は同じように定義できますか。'
  - オブジェクトリテラル型とはどういう意味ですか。
---
