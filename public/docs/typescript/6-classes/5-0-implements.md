---
id: typescript-classes-implements
title: 'implements: インターフェースによるクラスの形状の強制'
level: 2
question:
  - implements は具体的にどのような場面で役立ちますか？
  - インターフェースを実装するクラスで、そのインターフェースのメソッドを実装し忘れた場合、どのようなエラーが出ますか？
  - クラスが複数のインターフェースを実装する意味は何ですか？
  - インターフェースにはメソッドの中身（実装）を書かないのですか？
  - 'private title: string は、コンストラクタ引数でのプロパティ宣言の省略記法ですか？'
---

## `implements`: インターフェースによるクラスの形状の強制

[[./2]]で学んだインターフェースは、オブジェクトの型定義だけでなく、**クラスが特定の実装を持っていることを保証する（契約を結ぶ）**ためにも使われます。これを `implements` と呼びます。
