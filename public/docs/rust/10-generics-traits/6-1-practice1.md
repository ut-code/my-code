---
id: rust-generics-traits-practice1
title: '練習問題 1: ジェネリックなペア'
level: 3
question:
  - '`Pair<T, U>` 構造体の定義が分かりません。どのように書けば良いですか？'
  - '`new` メソッドはどのように実装すれば良いですか？'
  - '`print_pair` メソッドの中で `T` と `U` を表示するために `Debug` トレイトが必要な理由がわかりません。'
---

### 練習問題 1: ジェネリックなペア

2つの異なる型 `T` と `U` を保持できる構造体 `Pair<T, U>` を作成してください。
そして、その構造体にメソッド `new` （インスタンス作成）と、デバッグ出力をするメソッド `print_pair` を実装してください。
（ヒント：`print_pair` 内で `T` と `U` を表示するには、それぞれの型に `Debug` トレイトの制約が必要です）

```rust:practice11_1.rs



fn main() {
    let pair = Pair::new(10, "Hello");
    pair.print_pair();
}
```
```rust-exec:practice11_1.rs
10 and "Hello"
```
