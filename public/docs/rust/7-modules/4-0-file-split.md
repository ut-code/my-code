---
id: rust-modules-file-split
title: モジュールのファイル分割
level: 2
question:
  - ファイルを分割しないと、どのような問題が起こりますか？
  - '`mod xxx;`と書くだけで、なぜRustは別のファイルを探しに行くのですか？'
  - '`mod.rs`と`front_of_house.rs`は、どのように使い分けるのが一般的ですか？'
  - 他の言語のように、フォルダに入れるだけで自動的にモジュールとして認識されないのはなぜですか？
---
