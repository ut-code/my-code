---
id: rust-functions-control-if
title: if 式
level: 3
question:
  - ifブロックとelseブロックが返す型が同じである必要があるとありますが、もし異なる型を返そうとしたらどうなりますか？
  - if式で値を返さずに使用することもできますか？その場合、何が返されますか？
  - 条件式が必ずbool型でなければならないのはなぜですか？数値などを自動的に変換しないのは不便ではないですか？
---
