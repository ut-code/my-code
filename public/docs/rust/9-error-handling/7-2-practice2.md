---
id: rust-error-handling-practice2
title: '練習問題 2: データ処理チェーン（エラー伝播）'
level: 3
question:
  - 文字列をカンマで分割して最初の要素を取得するには、具体的にどのメソッドを使えば良いですか？
  - '`parse` メソッドが失敗した場合、そのエラーを `String` に変換するにはどうすれば良いですか？'
  - '`ok_or` メソッドはどのような時に使うのが適切ですか？'
---
