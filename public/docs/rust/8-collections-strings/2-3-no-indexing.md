---
id: rust-collections-strings-no-indexing
title: なぜインデックスアクセスができないのか？
level: 3
question:
  - なぜRustの文字列はs[0]のようにインデックスで直接アクセスできないのですか？他の言語ではできるのに。
  - UTF-8が可変長なことと、インデックスアクセスができないことの関係がよくわかりません。
  - 「意味のないバイト値」とは具体的にどういうことですか？
  - 文字列を「バイトとして見る」と「文字（スカラ値）として見る」の違いは何ですか？
  - .chars()と.bytes()は、それぞれどのような場面で使うべきですか？
  - '&s[0..3]で部分文字列が取得できるのに、なぜs[0]はエラーになるのですか？'
  - 「文字の境界に合わないバイトを指定すると実行時にパニックする」とは、具体的にどのような状況で起こりますか？
---
