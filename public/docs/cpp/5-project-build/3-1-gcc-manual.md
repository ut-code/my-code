...
level: 3
question:
  - '`-c` オプションと `-o` オプションはそれぞれ何を意味しますか？'
  - 生成された `main.o` や `math_utils.o` は、実行可能ファイルができてから削除しても良いですか？
  - 1回のg++コマンドでコンパイルとリンクを同時にできるなら、わざわざ分けて実行するのはどういう場合ですか？
---
...
または、以下のように1回のg++コマンドで複数[[ソースファイル]]のコンパイルとリンクを同時に行うこともできます。
