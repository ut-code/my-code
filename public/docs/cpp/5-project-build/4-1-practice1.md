...
`Calculator` という[[クラス]]を作成してください。このクラスは、加算、減算、乗算、除算の[[メンバ関数]]を持ちます。

* `Calculator.h`: `Calculator`クラスの定義を記述します。
* `Calculator.cpp`: 各[[メンバ関数]]の実装を記述します。
* `practice6_1.cpp`: `Calculator`クラスの[[インスタンス]]を作成し、いくつかの計算を行って結果を表示します。
