---
id: cpp-stl-algorithms-lambda
title: ラムダ式：その場で書ける無名関数
level: 2
question:
  - ラムダ式は通常の関数と何が違うのですか？
  - キャプチャ句の [=] と [&] はどう使い分ければ良いのですか？
  - ラムダ式で外部の変数を変更したい場合はどうすれば良いですか？
  - 戻り値の型 (-> 戻り値の型) はいつ省略できないのですか？
  - ラムダ式を使わないで降順ソートを実現する方法はありますか？
  - ラムダ式はstd::for_eachやstd::sort以外のアルゴリズムでも使えますか？
term:
  - ラムダ式
  - キャプチャ
  - キャプチャ句
...
```
  ...
  * `[]` **キャプチャ句**: ラムダ式の外にある[[変数]]を取り込んで、式の中で使えるようにします。
      * `[]`: 何もキャプチャしない。
      * `[=]`: 外の変数を全て[[値渡し]]（[[コピー]]）でキャプチャする。
      * `[&]`: 外の変数を全て[[参照渡し]]でキャプチャする。
      * `[x, &y]`: 変数 `x` は[[値渡し]]、変数 `y` は[[参照渡し]]でキャプチャする。
