...
イテレータは、コンテナ内の要素を指し示す「[[ポインタ]]のような」オブジェクトです。[[ポインタ]]のように `*` で要素の値を参照したり、`++` で次の要素に進んだりできます。
...
> [!TIP]
> [[範囲ベース for ループ]] では内部でイテレータが使われており、 [[`std::vector`]] などに限らずイテレータが取得できるオブジェクトに対して使用できます。
