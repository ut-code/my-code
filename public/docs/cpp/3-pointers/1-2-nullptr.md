...
level: 3
question:
  - 初期化されていないポインタが「不定な場所を指す」とは具体的にどういう意味か。何が危険なのか。
  - '`nullptr`、`NULL`、`0` はどれも「何も指していない」ことを表すように見えるが、なぜモダンC++では`nullptr`を使うべきなのか。'
  - ポインタを`nullptr`で初期化した後、後から他の変数のアドレスを指すように変更できるのか。
term:
  - nullptr
---
...
[[ポインタ]]が「どこも指していない」ことを示したい場合、C++では `nullptr` を使用します。
古いC++やC言語では `NULL` や `0` が使われていましたが、モダンC++では型安全な `nullptr` を使うのが鉄則です。[[初期化]]されていない[[ポインタ]]は不定な場所を指すため、必ず初期化しましょう。
