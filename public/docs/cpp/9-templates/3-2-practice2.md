---
id: cpp-templates-practice2
title: '練習問題2: 汎用的なスタッククラス'
level: 3
question:
  - pop()関数でスタックが空のときに呼び出すとどうなりますか エラー処理は必要ですか
  - pushやpop以外に、スタックに一般的に必要とされる機能はありますか
  - SimpleStackの要素数を取得するメンバ関数はどのように実装すればいいですか
  - std::vectorを内部で使う場合、popの実装はどのようにすればいいですか
  - std::runtime_errorを使うのはなぜですか 他の例外でもいいですか
---

### 練習問題2: 汎用的なスタッククラス

後入れ先出し（LIFO）のデータ構造であるスタックを、[[クラステンプレート]]`SimpleStack`として実装してください。以下の[[メンバ関数]]を持つようにしてください。
