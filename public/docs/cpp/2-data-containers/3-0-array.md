...
> [!NOTE]
> 固定長の配列として、[[Cスタイルの配列]] (`int coords[3];` など) もあります。しかし、Cスタイルの配列は、他のコンテナ（vectorなど）と操作感が異なり、サイズ情報を自分で管理しなければならないなどの欠点があります。`std::array` はC配列のパフォーマンス（[[スタック]]確保）と、コンテナの利便性（`.size()`などが使える）を両立させたものです。
