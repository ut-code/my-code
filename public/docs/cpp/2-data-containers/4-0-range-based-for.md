...
<!--上下のalertを分離するためのコメント-->

> [!WARNING]
> 以下のコードの `auto item` は、要素を「コピー」して取り出します。[[`std::string`]] のような大きなデータを扱う場合、コピーコストを避けるために `const auto& item` （参照）を使うのが一般的ですが、これについては[[./5]]で詳しく解説します。今の段階では「`auto` でループが回せる」と覚えておけば十分です。
