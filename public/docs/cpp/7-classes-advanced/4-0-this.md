...
非staticなメンバ関数が呼び出されるとき、その関数は「どのオブジェクトに対して呼び出されたか」を知る必要があります。コンパイラは、そのメンバ関数に対して、呼び出し元のオブジェクトの[[アドレス]]を暗黙的に渡します。このアドレスを保持するのが `this` ポインタです。

`this` は、メンバ関数内で使用できるキーワードで、自分自身のオブジェクトを指す[[ポインタ]]です。
