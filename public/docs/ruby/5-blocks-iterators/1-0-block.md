...
level: 2
question:
  - 「コードの塊」とは具体的にどのようなものですか？
  - do...end と {} の使い分けは、本当に複数行かどうかだけで、他には違いはないのですか？
  - メソッド呼び出しに「ブロックを渡す」とは、具体的にどのような操作を指すのでしょうか？
  - メソッド側が受け取ったコードの塊を「好きなタイミングで実行できる」とは、どういう意味ですか？
  - irb(main):001:0> の「irb」は何の略で、これは何を表しているのですか？
  - '=> 3 の「=>」は何を意味し、この場合はなぜ3が返ってくるのですか？'
---
