...
export function createWorkerRuntime(config: WorkerRuntimeConfig) {
  const RuntimeContextInternal = createContext<RuntimeContext>(null!);
  ...
  const Provider = ({ children }: { children: ReactNode }) => {
    ...
    // Generic postMessage
    function postMessage<T>(message: Omit<MessageToWorker, "id" | "payload"> & { payload: MessageToWorker['payload']}) {
    ...
    return (
      <RuntimeContextInternal.Provider
        value={{
          ready,
          runCommand,
          checkSyntax,
          ...
          runFiles,
          interrupt,
          splitReplExamples: config.splitReplExamples,
          getCommandlineStr: config.getCommandlineStr,
        }}
      >
        {children}
      </RuntimeContextInternal.Provider>
